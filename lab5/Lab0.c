...
void part5() {
    ...
    //Scores* class_grades_a = (Scores*) malloc(1);
	void * ptrPayload = malloc(16);
	void * ptrHeader = (void *)UNSCALED_POINTER_SUB(ptrPayload, WORD_SIZE);
	printf("ptrHeader = 0x%x\n", *(unsigned int*)ptrHeader);
    Scores* class_grades = (Scores*) malloc(sizeof(Scores) * 5);
	//printf("cga = %p\n", class_grades_a);
	printf("cg  = 0x%x\n\n", *(unsigned int*)class_grades);
